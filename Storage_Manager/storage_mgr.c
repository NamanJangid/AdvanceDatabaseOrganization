...
RC readBlock (int filePageNum, SM_FileHandle *smFileHandler, SM_PageHandle smMemPageHandler){
    ...
    if(strFilePointer != NULL) {
        ...
        if( filePageNum > totalNumberOfPgs - 1 || filePageNum < 0) {
             printf("Page doesn't exists");
    ...
    if(startingPosition){
        seekStart = fseek(strFilePointer, filePageNum *PAGE_SIZE* sizeof(char), SEEK_SET);
        if(seekStart != 0 ) {
            printf("Pointer at starting position") ;
        }else{
    ...
    int ind = fclose(strFilePointer); 
    if(ind == 0 ){
        return RC_OK;
    }else{
        return RC_message = "File Not Closed Properly"; 
    }
    fclose(strFilePointer);     	     
