...
RC readFileMetaData(BM_BufferPool *bm, BM_PageHandle *ph, FileMetaData *fmd,
                    int pageNumber) {
    if(pinPage(bm, ph, pageNumber) != RC_OK){
        return RC_CALLOC_FAILED;
    }
    char *ptr = ph->data;
    memcpy(fmd, ptr, sizeof(FileMetaData));
...
RC updateRootNode(BTreeHandle *tree, int pageNumber, KeyData kd) {
    ...
    float *childPointers = (float *)malloc(10 * sizeof(float));

    if(&kd != NULL){
        keys[0] = kd.key;
        childPointers[0] = kd.left;
        childPointers[1] = kd.right;
    }
 
    int nodeCnt = currNumNodes + 1;
    updateNodeData(tree, &rootNode, 0, nodeCnt, keys, childPointers, 1, -1);
    puts("Updating the node");
    treeMgmtData->fmd.numNodes++;
    treeMgmtData->fmd.rootPageNumber = currNumNodes + 1;
    if(treeMgmtData){
    writeBtreeNode(&rootNode);
    updateParentNodeInChildNodes(tree, rootNode);
    }
    return RC_OK;
}

RC updateParentNodeInChildNodes(BTreeHandle *tree, PageData node, TreeMgmtData *btreeMgmtData) {
    int i = 0; 
    while(i < node.numEntries + 1) {
        int childPageNum, childSlotNum;
        fetchIDPtr(node.pointers[i], &childPageNum, &childSlotNum);
        PageData childNode;
        RC rc = readPageData(btreeMgmtData->bufferPool, btreeMgmtData->pageHandle, &childNode, childPageNum);
        if (rc == RC_OK){        
        childNode.parentNode = node.pageNumber;
        //childNode.numEntries = node.numEntries;
        rc = writeBtreeNode(&childNode);
...
RC insertKeyInLeafNode(PageData *page, int key, RID rid) {

    int *newKeys = (int *)malloc(sizeof(int) * 10);
    int i = 0;
    float *newChildren = (float *)malloc(sizeof(int) * 10);
    
    while(i < page->numEntries && key > page->keys[i]) {
    ...
    newKeys[i] = key;
    page->numEntries++;
    newChildren[i] = rid.page + rid.slot * 0.1;
    i = i+1;
    while(i <= page->numEntries) {
        newKeys[i] = page->keys[i - 1];
        int j = i-1;
        newChildren[i] = page->pointers[j];
        i++;
    }
    free(page->keys);
    free(page->pointers);
